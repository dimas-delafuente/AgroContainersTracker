...
namespace AgroContainerTracker.Infrastructure.Validators
{
    public class ProductEntryValidator : AbstractValidator<ProductEntry>
    {
        ...
        private const string NAME_EXISTS_MESSAGE = "Ya existe este número de entrada para la campaña asignada.";
        public ProductEntryValidator(IProductEntryService productEntryService)
        {
            ...
            RuleFor(v => v.ProductEntryNumber)
                ...
                .MustAsync(async (entry, number, cancellation) =>
                {
                    ProductEntry productEntry = await productEntryService.GetAsync(entry.CampaingId, entry.ProductEntryNumber).ConfigureAwait(false);

                    return productEntry == null;
